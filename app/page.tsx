...
export default function Home() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      introTimeline
        .from(".hero-badge", { y: 24, autoAlpha: 0, duration: 0.5 })
        .from(".hero-title-line", { y: 56, autoAlpha: 0, stagger: 0.14 }, "-=0.2")
        .from(".hero-subheading", { y: 20, autoAlpha: 0, duration: 0.6 }, "-=0.35")
        .from(".hero-cta", { y: 18, autoAlpha: 0, stagger: 0.12, duration: 0.5 }, "-=0.35")
        .from(".hero-metric", { y: 14, autoAlpha: 0, stagger: 0.08, duration: 0.42 }, "-=0.25")
        .from(".hero-image-block", { y: 40, autoAlpha: 0, duration: 0.65 }, "-=0.45");
  ...
  return (
    <div
      ref={pageRef}
      className="relative min-h-screen overflow-x-hidden bg-gradient-to-br from-[#edf3ff] via-[#f8fbff] to-[#eef7ff] pb-14 pt-0 text-[#0f1f3a] [text-rendering:optimizeLegibility]"
    >
      <div
      ...
      <main className="relative z-[3] mx-auto w-full max-w-[1160px] px-4 sm:px-6 lg:px-10 xl:px-0">
        <section className="mt-10 grid items-center gap-8 lg:grid-cols-[1.06fr_0.94fr]">
          <div>
            <p className="hero-badge m-0 w-fit rounded-full border border-[rgba(26,97,255,0.22)] bg-[rgba(26,97,255,0.14)] px-[0.78rem] py-[0.4rem] text-[0.76rem] font-bold uppercase tracking-[0.04em] text-[#1a4cc8]">
              Parent + Teacher Sync, Day by Day
            </p>
            <h1 className="mt-[1.1rem] text-[clamp(2rem,4.3vw,4rem)] leading-[1.04] tracking-[-0.03em]">
              <span className="hero-title-line">Stay Connected to Your</span>
              <span className="hero-title-line">Child&apos;s School Journey</span>
            </h1>
            <p className="hero-subheading m-0 max-w-[44ch] text-[clamp(1rem,1.55vw,1.2rem)] leading-[1.65] text-[#32405e]">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <motion.a
                href="#features"
                className="hero-cta inline-flex h-[2.9rem] items-center justify-center rounded-[0.85rem] bg-gradient-to-br from-[#1a61ff] to-[#1348cc] px-[1.15rem] text-[0.94rem] font-[650] text-white no-underline shadow-[0_16px_28px_rgba(19,72,204,0.3)]"
                whileHover={{ y: -3, scale: 1.01 }}
                whileTap={{ scale: 0.98 }}
                transition={{ duration: 0.2 }}
              >
                Start With Juniotrack
              </motion.a>
              <motion.a
                href="#daily-flow"
                className="hero-cta inline-flex h-[2.9rem] items-center justify-center rounded-[0.85rem] border border-[rgba(18,36,76,0.14)] bg-white px-[1.15rem] text-[0.94rem] font-[650] text-[#0f1f3a] no-underline"
                whileHover={{ y: -3, scale: 1.01 }}
                whileTap={{ scale: 0.98 }}
                transition={{ duration: 0.2 }}
              >
            ...
            <div className="mt-[1.45rem] grid max-w-[620px] grid-cols-1 gap-[0.6rem] md:grid-cols-3">
              {metrics.map((metric) => (
                <div
                  className="hero-metric rounded-[0.9rem] border border-[rgba(18,36,76,0.14)] bg-white/70 px-[0.85rem] py-[0.8rem]"
                  key={metric.label}
                >
          ...
          <aside className="hero-image-block w-full max-w-[560px] justify-self-start opacity-100 lg:max-w-[540px] lg:justify-self-end">
            <Image
              src="/assets/kid.png"
